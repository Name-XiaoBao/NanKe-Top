  ...
  <van-pull-refresh v-model="loading" @refresh="onRefresh">
    <div style="padding-bottom: 10px">
      <div style="margin: 10px;">
        ...
        <br>
        <router-link v-for="voting_item in vote_item" :to="{path: '/vote',query: { uid: voting_item.uid }}"
                     style="color: #000;">
          <div class="voting">
            ...
            <div style="margin: 10px">
              ...
              <div class="parent" style="padding-bottom: 15px">
                ...
                <span class="right-span browse"><van-icon name="eye-o"
                                                          style="margin-right: 5px"/>{{ voting_item.views }}浏览</span>
                <span style="margin-left: 10px;font-size: 15px">已投<span
                    style="color:#0076ff;">{{ voting_item.votes }}</span>票</span>
              </div>
            </div>
          </div>
        </router-link>
...
<script>
import {ref} from "vue";
import {getCarouselList, getVoteByUsername, getVoteList} from '@/util/api.js';
...
export default {
  setup() {
    ...
    const vote_item = ref([]);
    const onRefresh = () => {
      getVoteByUsername(localStorage.getItem('username')).then(data => {
        // 处理返回的数据
